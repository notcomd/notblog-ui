<template>
  <Teleport to="body">
    <div class="fixed inset-0 z-[80] flex items-center justify-center p-4">
      <!-- 遮罩层 -->
      <div class="absolute inset-0 bg-black/40" aria-label="关闭" tabindex="0" @click="emit('close')" @keydown.enter.prevent="emit('close')" @keydown.space.prevent="emit('close')"></div>

      <!-- 面板 -->
      <div class="relative w-[min(430px,92vw)] max-h-[86vh] overflow-y-auto overscroll-contain glass-card p-5 space-y-4 skin-panel-in">
        <!-- 标题 -->
        <div class="flex items-center justify-between">
          <h3 class="text-base font-bold text-zinc-800 dark:text-zinc-100">皮肤设置</h3>
          <button class="w-8 h-8 rounded-[5%] flex items-center justify-center text-zinc-500 dark:text-zinc-300 hover:bg-white/60 dark:hover:bg-zinc-800/60 transition-colors" type="button" aria-label="关闭" title="关闭" @click="emit('close')">
        ...
        <div v-else class="space-y-4">
          ...
          <div class="space-y-2">
            ...
            <div class="flex gap-2">
              ...
              <button v-if="skin.customUrl" class="h-9 px-3 rounded-[5%] text-sm font-medium text-red-500 border border-red-300 dark:border-red-500/40 hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors" type="button" @click="skin.removeWallpaper()"><svg class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg> 移除背景</button>
            </div>
            <!-- 我的壁纸：点击切换回自定义背景 -->
            <div v-if="skin.customUrl" class="flex items-center gap-2.5">
              <button class="w-16 h-10 shrink-0 rounded-[5%] bg-cover bg-center border-2 transition-all" :class="skin.useCustom ? 'border-amber-400 ring-2 ring-amber-400/30' : 'border-transparent hover:border-white/50'" :style="customThumbStyle" type="button" title="我的壁纸" aria-label="我的壁纸" @click="skin.selectCustom()"></button>
          ...
          <div class="space-y-3 pt-1">
            <div>
              ...
              <input type="range" min="0" max="12" step="1" class="w-full accent-amber-500" aria-label="背景虚化模糊度" :value="skin.blur" @input="skin.setBlur(Number($event.target.value))" />
            ...
            <div>
              ...
              <input type="range" :min="skin.MIN_MASK" :max="skin.MAX_MASK" step="1" class="w-full accent-amber-500" aria-label="蒙层深浅" :value="skin.mask" @input="skin.setMask(Number($event.target.value))" />
