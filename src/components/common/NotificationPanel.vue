<template>
  <div class="absolute right-0 top-full mt-2 w-[min(20rem,90vw)] z-50 rounded-[5%] bg-white dark:bg-zinc-800/95 border border-zinc-200/70 dark:border-zinc-700/60 flex flex-col overflow-hidden" @click.stop>
    ...
    <div class="max-h-[70vh] overflow-y-auto overscroll-contain">
      <!-- 加载中 -->
      <div v-if="loading" class="py-10 flex justify-center" role="status">
      ...
      <div v-else>
        <div
          ...
          @keydown.enter.prevent="onItemClick(n, i)"
          @keydown.space.prevent="onItemClick(n, i)"
        >
          ...
          <div class="w-10 h-10 rounded-full flex items-center justify-center shrink-0" :class="meta(n).bg">
            <svg class="w-5 h-5" :class="meta(n).fg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" v-html="meta(n).icon"></svg>
          </div>
          ...
          <div class="min-w-0 flex-1">
            ...
            <button v-if="expanded[i] || truncatable[i]" class="text-[11px] text-amber-600 dark:text-amber-400 mt-0.5 hover:text-amber-700 transition-colors" type="button" :aria-expanded="expanded[i]" @click.stop="toggleExpand(i)">
              {{ expanded[i] ? '收起' : '展开' }}
            </button>
