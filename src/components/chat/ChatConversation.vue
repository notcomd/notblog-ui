  ...
  <div class="flex-1 min-w-0 glass-card flex flex-col min-h-0">
    ...
    <template v-else>
      ...
      <div class="flex items-center gap-3 px-5 py-3 border-b border-zinc-200/60 dark:border-zinc-700/60 shrink-0">
        <!-- 移动端返回会话列表（桌面端隐藏） -->
        <button class="lg:hidden shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-zinc-500 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-800/60 transition-colors" title="返回会话列表" aria-label="返回会话列表" @click="goBackList">
          <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="15 18 9 12 15 6" /></svg>
        </button>
        ...
        <template v-if="active && canCall">
          <button class="h-8 w-8 rounded-full bg-white/60 dark:bg-zinc-800/60 text-zinc-600 dark:text-zinc-300 hover:text-emerald-500 dark:hover:text-emerald-400 flex items-center justify-center transition-colors" title="语音通话" aria-label="语音通话" @click="startCall('Audio')">
            <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
          </button>
          <button class="h-8 w-8 rounded-full bg-white/60 dark:bg-zinc-800/60 text-zinc-600 dark:text-zinc-300 hover:text-amber-500 dark:hover:text-amber-400 flex items-center justify-center transition-colors" title="视频通话" aria-label="视频通话" @click="startCall('Video')">
            <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2" ry="2"/></svg>
          </button>
        </template>
      ...
      <!-- 群成员下拉 -->
      <div v-if="memberOpen" class="px-5 py-3 border-b border-zinc-200/60 dark:border-zinc-700/60 max-h-48 overflow-y-auto overscroll-contain">
          ...
          <div v-for="m in activeMessages" :key="m.messageId" class="flex" :class="isMine(m) ? 'justify-end' : 'justify-start'">
            <div class="max-w-[70%] rounded-[10px] px-3 py-2 text-sm"
              ...
              <div class="mt-1 flex items-center gap-2 text-[10px] opacity-70">
                ...
                <span v-else-if="isMine(m) && m.status === -1" class="text-red-200 cursor-pointer" @click="retryMessage(m)" role="button" tabindex="0" @keydown.enter.prevent="retryMessage(m)" @keydown.space.prevent="retryMessage(m)">发送失败，点击重试</span>
              </div>
            </div>
          </div>
        </template>
