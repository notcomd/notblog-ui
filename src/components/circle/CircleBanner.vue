<template>
  <!-- 频道 Banner 卡片：封面/头像/信息/操作按钮 + 分类 tab（公告·主页·资源·成员） -->
  <div>
    <div class="glass-card overflow-hidden">
      ...
      <div class="flex items-center gap-3 px-4 py-3">
        ...
        <button v-if="canManageUsers && !confirmingLeave" class="px-3 h-8 rounded-[5%] text-xs font-medium bg-gradient-to-r from-amber-400 to-orange-500 text-white hover:brightness-110 active:scale-95 transition-all" @click="$emit('manage')">⚙ 频道管理</button>
        <button v-if="!canManageUsers && !confirmingLeave && joinMode !== 'private'" class="px-3 h-8 rounded-[5%] text-xs font-medium text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10 hover:bg-amber-100 dark:hover:bg-amber-500/20 transition-colors" @click="genMemberInvite">🔗 邀请</button>
        <button v-if="!canManageUsers && !confirmingLeave" class="px-3 h-8 rounded-[5%] text-xs font-medium text-zinc-500 dark:text-zinc-300 hover:bg-white/60 dark:hover:bg-zinc-800/60 transition-colors" @click="confirmingLeave = true">退出频道</button>
...
<script setup>
// 频道 Banner：封面（图片/视频/头像兜底）+ 信息行 + 权限操作（管理/邀请/退出）+ 分类 tab
