      ...
      <div class="mt-4 pt-4 border-t border-zinc-200/60 dark:border-zinc-700/60">
        ...
        <div v-else class="space-y-1.5 max-h-44 overflow-y-auto overscroll-contain pr-0.5">
          <div v-for="inv in myInvites" :key="inv.inviteGuid" class="flex items-center gap-2 px-3 py-2 rounded-[5%] bg-white/40 dark:bg-zinc-800/40">
            ...
            <span v-if="inv.status === 'Accepted'" class="text-[11px] text-emerald-600 dark:text-emerald-400 shrink-0 inline-flex items-center gap-0.5"><svg aria-hidden="true" class="w-3 h-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>已接受</span>
