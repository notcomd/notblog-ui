<template>
  <div class="w-full max-w-md">
    ...
    <div class="transition-all duration-300">
      ...
      <!-- ==================== 登录方式切换（密码 / 邮箱验证码） ==================== -->
      <div class="mb-6 grid grid-cols-2 gap-1 p-1 rounded-[5%] bg-zinc-100/80 dark:bg-zinc-800/60">
        <button
          type="button"
          class="h-9 rounded-[5%] text-sm font-medium transition-all duration-200"
          :class="loginMode === 'password' ? 'bg-white dark:bg-zinc-700 text-amber-600 dark:text-amber-300 shadow-sm' : 'text-zinc-500 dark:text-zinc-300 hover:text-zinc-700 dark:hover:text-zinc-100'"
          @click="switchMode('password')"
        >密码登录</button>
        <button
          type="button"
          class="h-9 rounded-[5%] text-sm font-medium transition-all duration-200"
          :class="loginMode === 'code' ? 'bg-white dark:bg-zinc-700 text-amber-600 dark:text-amber-300 shadow-sm' : 'text-zinc-500 dark:text-zinc-300 hover:text-zinc-700 dark:hover:text-zinc-100'"
          @click="switchMode('code')"
        >验证码登录</button>
      </div>

      <!-- ==================== 密码登录 ==================== -->
      <form v-if="loginMode === 'password'" @submit.prevent="handlePasswordLogin" novalidate>
        <!-- 邮箱 -->
        <div class="mb-4">
          <label for="email" class="block text-xs font-semibold text-zinc-600 dark:text-zinc-200 tracking-wide mb-1.5">邮箱</label>
          <div class="relative">
        ...
        <div class="mb-4">
          <div class="flex justify-between items-center mb-1.5">
            <label for="password" class="block text-xs font-semibold text-zinc-600 dark:text-zinc-200 tracking-wide">密码</label>
            <button
              type="button"
              class="text-xs text-amber-600 dark:text-amber-400 hover:text-amber-700 dark:hover:text-amber-300 transition-colors duration-200 underline underline-offset-2 decoration-amber-300 dark:decoration-amber-500"
              @click="switchMode('code')"
            >忘记密码？用验证码登录</button>
          </div>
          <div class="relative">
      ...
      <form v-else @submit.prevent="handleCodeLogin" novalidate>
        <p class="mb-4 text-xs text-zinc-500 dark:text-zinc-300 leading-relaxed">
          输入邮箱获取验证码即可登录；未注册的邮箱将自动注册，并把初始密码发送到该邮箱。
        </p>
        <!-- 邮箱 -->
        <div class="mb-4">
          <label for="code-email" class="block text-xs font-semibold text-zinc-600 dark:text-zinc-200 tracking-wide mb-1.5">邮箱</label>
          <div class="relative">
            <span class="absolute inset-y-0 left-0 flex items-center pl-3 text-zinc-400 dark:text-zinc-400">
              <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="mb-4">
          ...
          <div class="flex gap-2">
            ...
            <button
              ...
              :disabled="codeCountdown > 0 || codeSending || loading"
    ...
    <div
      v-if="infoMessage"
      class="mt-4 p-3 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-400/30 rounded-[5%] flex items-start gap-2 animate-fade-in"
    >
      <svg class="h-4 w-4 text-emerald-500 dark:text-emerald-300 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        <path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/>
      </svg>
      <p class="text-xs text-emerald-700 dark:text-emerald-300">{{ infoMessage }}</p>
    </div>
  </div>
</template>
...
const handlePasswordLogin = async (): Promise<void> => {
  ...
  try {
    ...
    if (saveLoginResult(res && res.data)) {
      ...
      // 未开 2FA 密码错误；开启 2FA 需验证码 → 统一提示并引导切换验证码通道
      errorMessage.value = '邮箱或密码错误；若您已开启双重验证，请改用验证码登录'
    }
  } catch (err) {
    errorMessage.value = '邮箱或密码错误；若您已开启双重验证，请改用验证码登录'
...
const handleSendCode = async (): Promise<void> => {
  ...
  try {
    ...
    errorMessage.value = (err && err.response && err.response.data && err.response.data.error)
      || '验证码发送失败，请稍后重试'
...
const handleCodeLogin = async (): Promise<void> => {
  ...
  loading.value = true
  try {
    // 统一登录/注册：未注册邮箱自动注册并下发初始密码
    const res = await login({ email: email.value, code: code.value })
    const data = res && res.data
    if (saveLoginResult(data)) {
      auth.refreshUserFromToken()
      if (data.isNewUser) {
        infoMessage.value = '账号已自动注册，初始密码已发送到您的邮箱，点击下方按钮进入主页'
      }
      setTimeout(() => router.replace('/home'), data.isNewUser ? 1200 : 0)
    } else {
      errorMessage.value = '邮箱或验证码错误'
    }
  } catch (err: any) {
...
// ==================== 第三方登录（后端 OAuth） ====================
// 回调地址需在 Identity OAuthOptions.AllowedRedirectUris 白名单内；
// 授权完成后带 code/state 回到 /login，由 LoginPage 换取 Token
