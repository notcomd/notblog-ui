...
function renderInline(text: string): string {
  ...
  t = t.replace(
    /`([^`]+)`/g,
    '<code class="px-1.5 py-0.5 rounded-[5%] bg-zinc-200/80 dark:bg-zinc-700/60 text-[0.85em] font-mono">$1</code>'
  );
  // @提及
...
function renderTableRow(line: string, isHeader: boolean): string {
  ...
  return (
    ...
    cells
      .map(
        (c) =>
          `<${tag} class="px-3 py-2 border border-zinc-200 dark:border-zinc-700 ${isHeader ? 'bg-zinc-100 dark:bg-zinc-800 font-medium' : ''}">${renderInline(c)}</${tag}>`
      )
      .join('') +
    '</tr>'
  );
...
export function renderMarkdown(md: string | null | undefined): string {
  ...
  while (i < lines.length) {
    ...
    if (h) {
      ...
      out.push(
        `<h${level} class="${size} font-bold my-2.5 text-zinc-800 dark:text-zinc-100">${renderInline(h[2])}</h${level}>`
      );
      i++;
      continue;
    }
    ...
    if (/^\s*\|/.test(line) && i + 1 < lines.length && /^\s*\|?[\s:-]+\|/.test(lines[i + 1])) {
      ...
      if (body.length)
        html += '<tbody>' + body.map((b) => renderTableRow(b, false)).join('') + '</tbody>';
      html += '</table></div>';
      out.push(html);
      continue;
    }
    ...
    if (/^\s*>/.test(line)) {
      ...
      out.push(
        `<blockquote class="my-2 px-4 py-2.5 border-l-4 border-amber-400/70 bg-amber-400/10 rounded-r-[5%] text-zinc-600 dark:text-zinc-300">${renderInline(buf.join('<br>'))}</blockquote>`
      );
      continue;
    }
    ...
    if (/^\s*[-*+]\s+/.test(line)) {
      ...
      out.push(
        `<ul class="my-2 space-y-1 pl-5 list-disc">${buf.map((x) => `<li class="text-zinc-700 dark:text-zinc-200">${renderInline(x)}</li>`).join('')}</ul>`
      );
      continue;
    }
    ...
    if (/^\s*\d+\.\s+/.test(line)) {
      ...
      out.push(
        `<ol class="my-2 space-y-1 pl-5 list-decimal">${buf.map((x) => `<li class="text-zinc-700 dark:text-zinc-200">${renderInline(x)}</li>`).join('')}</ol>`
      );
      continue;
    }
    ...
    out.push(
      `<p class="my-2 leading-[1.8] text-zinc-700 dark:text-zinc-200">${renderInline(buf.join('<br>'))}</p>`
    );
  }
  return out.join('\n');
}
