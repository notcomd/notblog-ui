...
function defaults() {
  return { wallpaperId: null, customUrl: null, customFileId: null, useCustom: false, blur: DEFAULT_BLUR, mask: DEFAULT_MASK }
}

// v1 格式 { wallpaperId, blur, mask } → v2 兼容迁移（wallpaperId 即预设壁纸 id）
function loadPersisted() {
  try {
    const raw = JSON.parse(localStorage.getItem(SKIN_KEY) || '{}')
    const d = defaults()
...
export const useSkinStore = defineStore('skin', () => {
  ...
  const wallpaperId = ref(saved.wallpaperId) // 官方预设壁纸 id
  const customUrl = ref(saved.customUrl) // 自定义背景图 URL（fileUri 或本地 dataURI 兜底）
  const customFileId = ref(saved.customFileId) // 后端 FileRef.FileId（审核/删除用）
  const useCustom = ref(saved.useCustom && !!saved.customUrl) // 当前是否应用自定义图
  const blur = ref(saved.blur)
  const mask = ref(saved.mask)
  ...
  const wallpaper = computed(() => {
    ...
    const w = wallpaperId.value ? wallpaperById(wallpaperId.value) : null
    return w ? { id: w.id, name: w.name, url: wallpaperDataUri(w) } : null
  })
  const wallpaperUrl = computed(() => (wallpaper.value ? wallpaper.value.url : ''))
  ...
  async function setCustomWallpaper(file) {
    ...
    persist()
    toast.push(localOnly ? '已应用（本地预览，未登录或后端不可用时仅本机可见）' : '自定义背景已应用', 'success')
    return true
  }
  ...
  return {
    wallpaperId, customUrl, customFileId, useCustom,
    wallpaper, wallpaperUrl, blur, mask, effectiveMask,
    setWallpaper, setCustomWallpaper, selectCustom, setBlur, setMask, removeWallpaper,
    MIN_MASK, MAX_MASK,
  }
})
