...
/** 发布到社区请求体 */
interface CreateCirclePostData {
  circleGuid: string;
  content: string;
  fileIds?: string[];
  linkUrl?: string | null;
...
export function createCirclePost(payload: CreateCirclePostData) {
...
export function uploadImage(file: File, description = '') {
...
async function uploadVideoByChunks(file: File, onProgress?: (percent: number) => void) {
